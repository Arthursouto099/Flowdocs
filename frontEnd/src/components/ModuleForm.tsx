...
import type { Module, ModuleCreateInputs } from "../types/ApiTypes";
...
export default function ModuleForm({data, onClose}: {data?: Module, onClose: () => void }) {
    const [module, setModule] = useState<ModuleCreateInputs>({name: "", description: ""})
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
          ...
          try {
            const request = await axios.post("http://localhost:3000/module/create", {
            name: module?.name,
    ...
    return (
          ...
          <form onSubmit={onSubmit} className="grid grid-cols-2 gap-4 w-96 p-6  rounded shadow">
                            ...
                            <label className="flex flex-col col-span-2">
                                <span className="text-sm font-semibold mb-1">Nome (Modulo)</span>
                                <input
                                    type="text"
                                    name="name"
                                    onChange={(e) => {
                            ...
                            >
                                Cadastrar Modulo <FaArchive size={20}/>
                            </button>
                        </form>
